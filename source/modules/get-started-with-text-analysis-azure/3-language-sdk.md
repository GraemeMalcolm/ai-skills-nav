...
> **NOTE**: **Loading environment variables**: In this sample, `dotenv` (`load_dotenv()`) reads your `.env` file and loads those values into your app's environment. The `os` package then retrieves each value by name with `os.getenv()`, such as `os.getenv("AZURE_OPENAI_ENDPOINT")`.
>
