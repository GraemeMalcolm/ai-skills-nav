...
description: >-
  Learn when and how to use Retrieval Augmented Generation (RAG) to ground a language model with domain-specific data
...
![Diagram showing an ungrounded model returning an uncontextualized response based only on training data.](media/ungrounded.png)
...
![Diagram comparing an ungrounded model returning generic responses versus a grounded model returning data-backed responses.](media/grounded.png)
...
![Diagram showing the three-step RAG pattern: retrieve grounding data, augment the prompt with that data, and generate a grounded response.](media/rag-pattern.png)
...
![Diagram showing text keywords plotted as vectors in multidimensional space, with the distance between vectors representing semantic similarity.](media/vector-embeddings.jpg)
...
![Diagram showing an Azure AI Search index being queried to retrieve grounding data for a user question.](media/index.png)
