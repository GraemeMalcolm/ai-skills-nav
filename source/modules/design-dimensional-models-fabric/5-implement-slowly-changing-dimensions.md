---
title: Implement slowly changing dimensions
description: Learn patterns for implementing slowly changing dimensions in analytical data stores
---
...
> **NOTE**: Type 1 changes can affect historical analysis. If a salesperson is reassigned to a new region and you overwrite the region, all their past sales appear under the new region. Consider whether this behavior meets your reporting requirements.
...
![Diagram that shows SCD Type 2 before and after a customer's region change, with a new row added and the original row expired.](media/slowly-changing-dimension-type-2.svg)
...
> **TIP**: Avoid applying Type 2 tracking to every attribute in a dimension. Only track history on attributes where the business requires historical analysis. Use Type 1 for the rest to keep the dimension manageable.
