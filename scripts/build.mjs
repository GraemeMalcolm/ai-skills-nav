...
function courseOverview(outputFile, course, playlists) {
  ...
  const nextTarget = firstPlaylist
    ...
    : null;
  return overview(outputFile, course, "courses", credentialSection, playlistList, pageNavigation(outputFile, null, nextTarget), true);
}
...
async function build() {
  ...
  const courseTools = `<div class="header-tools">${courseSearch}<a class="filter-trigger" href="${relativeUrl(coursesFile, homeFile)}#catalog-filter">Filter<span class="filter-count" data-filter-count hidden></span></a></div>`;
  ...
  await writePage(coursesFile, shell({ outputFile: coursesFile, title: "Courses", breadcrumbs: [{ label: "Courses" }], headerExtra: courseTools, avatar: defaultAvatar, content: coursesContent, bodyClass: "catalog-page" }));

  const playlistSearch = catalogSearch("playlist-search-input", "Search playlists", "Search playlists");
  const playlistTools = `<div class="header-tools">${playlistSearch}<a class="filter-trigger" href="${relativeUrl(playlistsFile, homeFile)}#catalog-filter">Filter<span class="filter-count" data-filter-count hidden></span></a></div>`;
  ...
  await writePage(playlistsFile, shell({ outputFile: playlistsFile, title: "Skilling playlists", breadcrumbs: [{ label: "Skilling playlists" }], headerExtra: playlistTools, avatar: defaultAvatar, content: playlistsContent, bodyClass: "catalog-page" }));

  const moduleSearch = catalogSearch("module-search-input", "Search skilling content", "Search skilling content");
  const moduleTools = `<div class="header-tools">${moduleSearch}<a class="filter-trigger" href="${relativeUrl(skillingContentFile, homeFile)}#catalog-filter">Filter<span class="filter-count" data-filter-count hidden></span></a></div>`;
  ...
  for (const playlist of playlists) {
    ...
    await writePage(playlistFile, shell({ outputFile: playlistFile, title: playlist.title, breadcrumbs: playlistBreadcrumbs, sidebar, avatar: playlist.avatarData, bodyClass: "learning-page", content: overview(playlistFile, playlist, "playlists", "", moduleList, playlistNavigation, true) }));
  ...
  for (const course of courses) {
    ...
    for (const [playlistIndex, playlist] of coursePlaylists.entries()) {
      ...
      await writePage(playlistFile, shell({ outputFile: playlistFile, title: playlist.title, breadcrumbs: playlistBreadcrumbs, sidebar: courseSidebar(playlistFile, course, coursePlaylists, playlist.slug), avatar: playlist.avatarData, bodyClass: "learning-page", content: overview(playlistFile, playlist, "playlists", "", moduleList, playlistNavigation, true) }));
