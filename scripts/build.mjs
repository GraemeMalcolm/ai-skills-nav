...
function courseOverview(outputFile, course, playlists, credentials) {
  ...
  const credentialContent = credentials.length
    ? `<ul class="credential-resources">${credentials.map((credential) => `<li${accessData(credential)}><a href="${relativeUrl(outputFile, path.join(outputRoot, "credentials", credential.slug, "index.html"))}">
      ...
      <strong>${escapeHtml(credential.credential_type || "Credential")}: ${escapeHtml(credential.title)}</strong>
    </a></li>`).join("")}</ul>`
...
async function build() {
  ...
  const catalogTools = `<div class="catalog-section-tools catalog-intro-tools">${catalogSearchForm}<button class="filter-trigger" type="button" data-filter-open>Filter<span class="filter-count" data-filter-count hidden></span></button></div>`;
  const catalogContent = `<section class="catalog-intro hero-theme-catalog"><p class="kicker">Explore all learning</p><h1>Catalog</h1><p>Browse courses, skilling playlists, and individual learning experiences.</p>${catalogTools}</section>
    <section class="catalog-section catalog-results"><div class="card-grid catalog-card-grid" data-paged-catalog>${catalogItems.map(({ item, type }) => card(catalogFile, item, type)).join("")}</div><p class="filter-empty" data-catalog-empty role="status" aria-live="polite" hidden>No skilling matches your search and filters.</p><nav class="catalog-pagination" aria-label="Catalog pages" data-catalog-pagination></nav></section>
    ${catalogFilterDialog(discoverableItems, ["role", "experience_type", "level", "duration", "modalities"], "the catalog")}`;
  ...
  const officialTools = `<div class="catalog-section-tools catalog-intro-tools">${officialSearch}<button class="filter-trigger" type="button" data-filter-open>Filter<span class="filter-count" data-filter-count hidden></span></button></div>`;
  ...
  const credentialTools = `<div class="catalog-section-tools catalog-intro-tools">${credentialSearch}<button class="filter-trigger" type="button" data-filter-open>Filter<span class="filter-count" data-filter-count hidden></span></button></div>`;
  const credentialsContent = `<section class="catalog-intro hero-theme-credentials"><p class="kicker">Validate your skills</p><h1>Credentials</h1><p>Verified, high-value credentials that employers trust. Bridging the gap for both technical and business audiences.</p>${credentialTools}</section>
    <section class="catalog-section catalog-results"><div class="card-grid catalog-card-grid" data-paged-catalog>${credentials.map((item) => card(credentialsFile, item, "credentials")).join("")}</div><p class="filter-empty" data-catalog-empty role="status" aria-live="polite" hidden>No credentials match your search and filters.</p><nav class="catalog-pagination" aria-label="Credential pages" data-catalog-pagination></nav></section>
  ...
  const personalizedPlanTools = `<div class="catalog-section-tools catalog-intro-tools personalized-plan-tools">${personalizedPlanSearch}<button class="filter-trigger" type="button" data-filter-open>Filter<span class="filter-count" data-filter-count hidden></span></button></div>`;
  ...
  const personalizedPlanContent = `${personalPlaylistDetail}<div data-personalized-plan data-auth-only data-playlists-url="${relativeUrl(personalizedPlanFile, personalizedPlanFile)}" data-playlist-thumbnail="${relativeUrl(personalizedPlanFile, path.join(outputRoot, "assets", "my-playlist.png"))}">
    <section class="catalog-intro hero-theme-skilling"><p class="kicker">Personalized learning</p><h1>My skilling plan</h1><p data-personalized-summary>Your recommendations are based on the role selected in your profile.</p>${personalizedPlanTools}</section>
    <section class="catalog-section" data-continue-section hidden><div class="section-heading"><p class="kicker">Resume learning</p><h2>Continue where you left off</h2></div><div class="card-grid" data-continue-grid></div>${continueTemplates}</section>
    ...
    ${catalogFilterDialog(discoverableItems, ["role", "modalities"], "your skilling plan")}
  </div>`;
