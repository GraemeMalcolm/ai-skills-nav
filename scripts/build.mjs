...
async function build() {
  ...
  const catalogContent = `<section class="catalog-intro hero-theme-catalog"><p class="kicker">Explore all learning</p><h1>Catalog</h1><p>Browse courses, skilling playlists, and individual learning experiences.</p></section>
  ...
  const credentialsContent = `<section class="catalog-intro hero-theme-credentials"><p class="kicker">Validate your skills</p><h1>Credentials</h1><p>Verified, high-value credentials that employers trust. Bridging the gap for both technical and business audiences.</p></section>
  ...
  const personalizedPlanContent = `${personalPlaylistDetail}<div data-personalized-plan data-auth-only data-playlists-url="${relativeUrl(personalizedPlanFile, personalizedPlanFile)}" data-playlist-thumbnail="${relativeUrl(personalizedPlanFile, path.join(outputRoot, "assets", "my-playlist.png"))}">
    <section class="catalog-intro hero-theme-skilling"><p class="kicker">Personalized learning</p><h1>My skilling plan</h1><p data-personalized-summary>Your recommendations are based on the role selected in your profile.</p></section>
