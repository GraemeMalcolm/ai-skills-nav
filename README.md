...
- **Courses**: Microsoft Official Curricula that can be completed as self-paced learning or delivered as instructor-led training. A course includes one or more playlists and can be aligned to multiple credentials.
  - **Playlists**: Ordered, curated collections containing one or more modules (equivalent of *Learning Paths* in Microsoft Learn). Learners can also create *personal* playlists containing modules of their choice.
    - **Modules**: The core learning assets. Metadata describes each module's level, duration, topics, role, pages, and optional avatar; modalities are derived at build time from page directives.
...
Signed-in users can open **Profile** to view their email address and choose a primary role, other roles of interest, and preferred content modalities. Saving opens **My Skilling**, whose active filters initialize from those preferences on every visit. Primary-role recommendations always use the saved role, and other-role recommendations use selected non-primary roles while excluding items that also target the primary role; both apply the selected modalities but ignore other metadata filters. Organization recommendations show all skilling assigned to the signed-in domain, and personal playlists show every saved playlist; neither section is filtered by search or metadata.
